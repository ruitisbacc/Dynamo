...
#include "network/kryo_client.hpp"
#include "network/kryo_serializer.hpp"
#include "network/packet_registry.hpp"
...
void GameEngine::update() {
    ...
    auto pruneDelta = std::chrono::duration_cast<std::chrono::milliseconds>(
        now - lastPrune_).count();
    if (pruneDelta >= 1000) {  // Prune every second
        pruneStaleEntities();
        lastPrune_ = now;
    }
...
void GameEngine::switchAmmo(int32_t ammoType) {
    if (!canSend()) return;
    if (ammoType < 1 || ammoType > 6) return;
...
void GameEngine::switchRocket(int32_t rocketType) {
    if (!canSend()) return;
    if (rocketType < 1 || rocketType > 3) return;
...
void GameEngine::switchConfig(int32_t configIndex) {
    if (!canSend()) return;
    if (configIndex != 1 && configIndex != 2) return;
...
void GameEngine::logout() {
    if (state_ != EngineState::InGame) return;
...
void GameEngine::handleApiResponse(const std::string& uri, const std::string& jsonData) {
    try {
        if (uri == WuApiUri::TOKEN_LOGIN) {
            ...
            {
                std::lock_guard<std::mutex> lock(stateMutex_);
                if (j.contains("name")) hero_.name = j["name"].get<std::string>();
                if (j.contains("level")) hero_.level = j["level"].get<int32_t>();
                if (j.contains("fraction")) profileFraction = j["fraction"].get<int32_t>();
            }
    ...
    try {
        if (key == "map-info" || key == WuApiUri::MAP_INFO) {
            ...
            std::optional<int32_t> playerFraction;
            {
                ...
                if (j.contains("name")) hero_.name = j["name"].get<std::string>();
                if (j.contains("level")) hero_.level = j["level"].get<int32_t>();
                if (j.contains("fraction")) playerFraction = j["fraction"].get<int32_t>();
                if (j.contains("btc")) hero_.btc = j["btc"].get<int64_t>();
                if (j.contains("plt")) hero_.plt = j["plt"].get<int64_t>();
            }
...
void GameEngine::handleMapInfoNotification(const std::string& jsonData) {
    try {
        ...
        MapInfo callbackMapInfo;
        {
            ...
            if (j.contains("id")) mapInfo_.id = j["id"].get<int32_t>();
            if (j.contains("name")) mapInfo_.name = j["name"].get<std::string>();
            if (j.contains("width")) mapInfo_.width = j["width"].get<int32_t>();
            if (j.contains("height")) mapInfo_.height = j["height"].get<int32_t>();
            if (j.contains("factionId")) mapInfo_.fraction = j["factionId"].get<int32_t>();
            if (j.contains("fraction")) mapInfo_.fraction = j["fraction"].get<int32_t>();
            if (j.contains("pvp")) mapInfo_.pvp = j["pvp"].get<bool>();
            ...
            if (config_.logPackets && !ship.changes.empty()) {
                ...
                for (const auto& c : ship.changes) {
                    if (c.id == ParamId::USERNAME) {
                        if (std::holds_alternative<std::string>(c.data)) shipName = std::get<std::string>(c.data);
                    }
                }
        ...
        std::optional<int32_t> userInfoFraction;
        {
            ...
            for (const auto& param : packet.params) {
                ...
                switch (param.id) {
                    case ParamId::BTC:
                        hero_.btc = value;
                        if (config_.logPackets) appendEngineLog(std::format("[UserInfo] BTC: {}", value));
                        break;
                    case ParamId::PLT:
                        hero_.plt = value;
                        if (config_.logPackets) appendEngineLog(std::format("[UserInfo] PLT: {}", value));
                        break;
                    case ParamId::EXPERIENCE:
                        hero_.experience = value;
                        if (config_.logPackets) appendEngineLog(std::format("[UserInfo] EXP: {}", value));
                        break;
                    case ParamId::HONOR:
                        hero_.honor = static_cast<int32_t>(value);
                        if (config_.logPackets) appendEngineLog(std::format("[UserInfo] Honor: {}", value));
                        break;
                    case ParamId::LEVEL:
                        hero_.level = static_cast<int32_t>(value);
                        if (config_.logPackets) appendEngineLog(std::format("[UserInfo] Level: {}", value));
                        break;
                    case ParamId::BOOTY_KEYS:
                        hero_.bootyKeys = static_cast<int32_t>(value);
                        if (config_.logPackets) appendEngineLog(std::format("[UserInfo] Keys: {}", value));
                        break;
                    ...
                    case ParamId::MAP_ID:
                        ...
                        if (config_.logPackets) appendEngineLog(std::format("[UserInfo] Map changed to ID: {}", value));
                        break;
                    case ParamId::FACTION:
                        if (config_.logPackets) appendEngineLog(std::format("[UserInfo] Faction: {}", value));
